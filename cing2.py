'''Train a simple deep CNN on the CIFAR10 small images dataset.

GPU run command with Theano backend (with TensorFlow, the GPU is automatically used):
    THEANO_FLAGS=mode=FAST_RUN,device=gpu,floatx=float32 python cifar10_cnn.py

It gets down to 0.65 test logloss in 25 epochs, and down to 0.55 after 50 epochs.
(it's still underfitting at that point, though).
'''

from __future__ import print_function
import keras
from keras.datasets import cifar10
from keras.preprocessing.image import ImageDataGenerator
from keras.models import Sequential
from keras.layers import Dense, Dropout, Activation, Flatten
from keras.layers import Conv2D, MaxPooling2D
import h5py
import os

from keras import backend as K

import numpy as np
from keras.preprocessing.image import load_img,img_to_array,array_to_img

def dataget():
    sagiridataNum = 500
    nosagiridataNum = 500
    testzise = 50
    sagiri = np.ones((sagiridataNum, 270 ,480,3))
    sagiri_ = np.ones((sagiridataNum,1))
    for idx in range(sagiridataNum - testzise):
        img = load_img("HDanime\sagiri (" + str(idx + 1) + ").jpg",target_size=(270 ,480))
        nparreay = img_to_array(img)
        sagiri[idx] = nparreay
    nosagiri = np.ones((nosagiridataNum, 270 ,480 ,3))
    nosagiri_ = np.zeros((nosagiridataNum,1))
    for idx in range(nosagiridataNum - testzise):
        img = load_img("HDeroge\osagiri (" + str(idx + 1) + ").jpg",target_size=(270 ,480))
        nparreay = img_to_array(img)
        nosagiri[idx] = nparreay
    alldata =  np.concatenate([sagiri,nosagiri],axis=0)
    alldata_ =  np.concatenate([sagiri_,nosagiri_],axis=0)

    testsagiri = np.ones((testzise, 270 ,480 ,3))
    testsagiri_ = np.ones((testzise,1))
    i = 0
    for idx in range(sagiridataNum - testzise , sagiridataNum):
        img = load_img("HDanime\sagiri (" + str(idx + 1) + ").jpg",target_size=(270 ,480))
        nparreay = img_to_array(img)
        testsagiri[i] = nparreay
        i+= 1
    testnosagiri = np.ones((testzise, 270 ,480,3))
    testnosagiri_ = np.zeros((testzise,1))
    i = 0
    for idx in range(nosagiridataNum - testzise , nosagiridataNum):
        img = load_img("HDeroge\osagiri (" + str(idx + 1) + ").jpg",target_size=(270 ,480))
        nparreay = img_to_array(img)
        testnosagiri[i] = nparreay
        i+=1
    testalldata =  np.concatenate([testsagiri,testnosagiri],axis=0)
    testalldata_ =  np.concatenate([testsagiri_,testnosagiri_],axis=0)
    #imga = array_to_img(testalldata[60])
    #imga.show()

    #print('shape:', testalldata.shape)
    return (alldata, alldata_), (testalldata, testalldata_)

def datagetBase(path,classAattribute):
    for idx in range(10000):
        img = load_img(path + str(idx + 1) + ").jpg",target_size=(270 ,480))
        nparreay = img_to_array(img)
        sagiri = nparreay.astype('float32')
        sagiri /= 255
        #classAattribute = keras.utils.to_categorical(classAattribute, 2)
        classAattribute2 = np.ones(2)
        classAattribute2[1] = 0
        yield (sagiri,classAattribute2)

def datagetTrain():
    s1 = datagetBase("HDeroge\osagiri (",0)
    s2 = datagetBase("HDanime\sagiri (",1)
    for idx in range(255):
        yield s1.__next__()
        yield s2.__next__()
#K.set_image_dim_ordering('th')

batch_size = 32
num_classes = 2
epochs = 4
data_augmentation = False

ImageDataGenerator.LOAD_TRUNCATED_IMAGES = True

# The data, shuffled and split between train and test sets:
#(x_train, y_train), (x_test, y_test) = dataget()#cifar10.load_data()
#print('x_train shape:', x_train.shape)
#print(x_train.shape[0], 'train samples')
#print(x_test.shape[0], 'test samples')

# Convert class vectors to binary class matrices.
#y_train = keras.utils.to_categorical(y_train, num_classes)
#y_test = keras.utils.to_categorical(y_test, num_classes)

model = Sequential()

model.add(Conv2D(32, (3, 3), padding='same',
                 input_shape=(270 ,480,3)))
model.add(Activation('relu'))
model.add(Conv2D(32, (3, 3)))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Dropout(0.25))

model.add(Conv2D(64, (3, 3), padding='same'))
model.add(Activation('relu'))
model.add(Conv2D(64, (3, 3)))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Dropout(0.25))

model.add(Flatten())
model.add(Dense(512))
model.add(Activation('relu'))
model.add(Dropout(0.5))
model.add(Dense(num_classes))
model.add(Activation('softmax'))

# initiate RMSprop optimizer
opt = keras.optimizers.rmsprop(lr=0.0001, decay=1e-6)

# Let's train the model using RMSprop
model.compile(loss='categorical_crossentropy',
              optimizer=opt,
              metrics=['accuracy'])

#x_train = x_train.astype('float32')
#x_test = x_test.astype('float32')
#x_train /= 255
#x_test /= 255

#tb_cb = keras.callbacks.TensorBoard(log_dir='./logs', histogram_freq=0, write_graph=True)

if not data_augmentation:
    print('Not using data augmentation.')
    #model.fit(x_train, y_train,
    #          batch_size=batch_size,
    #          epochs=epochs,
    #          validation_data=(x_test, y_test),
    #          shuffle=True)
    #model.fit_generator(datagetTrain(),
    #                steps_per_epoch = 500 , epochs= 20)
    train_datagen = ImageDataGenerator(
        rescale=1./255,
        shear_range=0.2,
        zoom_range=0.2,
        horizontal_flip=True)

    test_datagen = ImageDataGenerator(rescale=1./255)

    train_generator = train_datagen.flow_from_directory(
        'imageGene',
        target_size=(270 ,480),
        batch_size=8,
        shuffle=True,
        class_mode='categorical')

    validation_generator = test_datagen.flow_from_directory(
        'imageGentest',
        target_size=(270 ,480),
        batch_size=8,
        class_mode='categorical')

    model.fit_generator(
        train_generator,
        steps_per_epoch =432,
        epochs=5,
        validation_data=validation_generator,
        validation_steps = 2)
else:
    print('Using real-time data augmentation.')
    # This will do preprocessing and realtime data augmentation:
    datagen = ImageDataGenerator(
        featurewise_center=False,  # set input mean to 0 over the dataset
        samplewise_center=False,  # set each sample mean to 0
        featurewise_std_normalization=False,  # divide inputs by std of the dataset
        samplewise_std_normalization=False,  # divide each input by its std
        zca_whitening=False,  # apply ZCA whitening
        rotation_range=0,  # randomly rotate images in the range (degrees, 0 to 180)
        width_shift_range=0.1,  # randomly shift images horizontally (fraction of total width)
        height_shift_range=0.1,  # randomly shift images vertically (fraction of total height)
        horizontal_flip=True,  # randomly flip images
        vertical_flip=False)  # randomly flip images

    # Compute quantities required for feature-wise normalization
    # (std, mean, and principal components if ZCA whitening is applied).
    datagen.fit(x_train)

    # Fit the model on the batches generated by datagen.flow().
    model.fit_generator(datagen.flow(x_train, y_train,
                                     batch_size=batch_size,shuffle = True),
                        steps_per_epoch=x_train.shape[0] // batch_size,
                        epochs=epochs,
                        validation_data=(x_test, y_test),
                        callbacks= [tb_cb])
model.save('testhozon.h5')
